...
module.exports = {
    name: "setup",
    description: "Runs some background checks and makes sure all settings are correct.",
    minArgs: 0,
    maxArgs: 0,
    commandPattern: "setup",
    category: "Utility",
    async execute (msg: Message, args: Array<string>, client: Client) {
        const guild = client.guilds.cache.get(config.config.discord.serverId) as Guild;
        const logChannel = guild.channels.cache.get(config.config.discord.logChannel) as TextChannel;
