...
for (let i = 0; i < commandArray.length; i++) {
  ...
  table.addRow(
    `${cmdToImport.name}`,
    cmdToImport.description,
    cmdToImport.minArgs,
    cmdToImport.maxArgs,
    cmdToImport.category
  )
}
...
export default (client: Client, statcord: SClient): void => {
  ...
  client.on("messageCreate", async (msg: Message) => {
    if (msg.mentions.has(client.user as User))
      return msg.reply(
        `Heyo! My prefix is \`${config.discord.botPrefix}\` - We are working on pinging, just not quite there yet!`
      )
    ...
    if (commandArray.includes(command)) {
      ...
      if (
        commandToRun.requiredPermissions ||
        !commandToRun.requiredPermissions === undefined
      ) {
        if (config.discord.botOwners.includes(msg.author.id)) {
          commandToRun.execute(msg, args, client)
          return statcord.postCommand(command, msg.author.id)
        ...
        for (let i = 0; i < commandToRun.requiredPermissions.length; i++) {
          if (
            !member!.permissions.has(`${commandToRun.requiredPermissions[i]}`)
          )
            return msg.reply(config.responses.noPermission)
        }
      ...
      await msg.reply(
        "Sorry! Command was not found. Please open a ticket if this is incorrect."
      )
    }
  })
  ...
  client.on("error", async (e) => {
    error(
      `SOMETHING REALLY REALLY WENT WRONG. HERE'S THE RUNDOWN: \n\n\`${e}\``,
      { sendWebhook: true }
    )
  })
}
